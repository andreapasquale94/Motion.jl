...
@inline function libration_point(μ::Number, val::Val{S}, tol = 1e-14) where S
...
function libration_points(μ::Number; tol = 1e-14)
	return [
		libration_point(μ, Val(:L1), tol),
		libration_point(μ, Val(:L2), tol),
		libration_point(μ, Val(:L3), tol),
		libration_point(μ, Val(:L4), tol),
		libration_point(μ, Val(:L5), tol),
	]
end
